...
\begin{tikzpicture}[
    ...
    dasharr/.style={-{Stealth[length=4pt]}, thick, dashed}
]
    \node[box] (game)      {Game Code};
    \node[box, right=of game] (dbi)  {DBI};
    \node[box, above right=0.6cm and 1.4cm of dbi] (remote) {Remote SQL\\(online)};
    \node[box, below right=0.6cm and 1.4cm of dbi] (sqlite) {SQLiteDBI\\+ CommandLog\\(offline)};
    \node[box, right=4.8cm of dbi] (replay) {ReplayAll};
    \node[box, right=of replay] (server) {Server};
    \node[box, below=0.9cm of replay] (manifest) {Rejection\\Manifest};

    \draw[arr] (game)   -- node[above,font=\tiny]{action} (dbi);
    \draw[arr] (dbi)    -- (remote);
    \draw[arr] (dbi)    -- (sqlite);
    \draw[dasharr] (sqlite) -- node[above,font=\tiny]{reconnect} (replay);
    \draw[arr] (replay) -- (server);
    \draw[arr] (server) -- node[right,font=\tiny]{result} (manifest);
\end{tikzpicture}
