...
Many survival games store their world entirely on remote servers, which means the moment a player loses their internet connection, progress stops. This thesis looks at how that limitation can be overcome — specifically, how a game can keep running offline and then safely merge what happened back into the shared world when the player reconnects.

The core idea is to record each player action as a small, self-contained command object rather than immediately writing to the server. While offline, these commands queue up locally. When connectivity returns, they are replayed against the authoritative server in order. The tricky part is deciding what to do when the server's world has moved on in the meantime: another player may have demolished the same building, or claimed the same territory. The thesis addresses this through a conflict resolution strategy that classifies commands by type — some can always be applied regardless of what changed, some need a version check, and some must simply be declined and reported back to the player.

A prototype was built in C++ to validate the design, including a SQLite-backed offline database and a durable command log that survives crashes. All components are tested with 31 automated tests.

\section*{Sammendrag}

Mange overlevelsesspill lagrer hele spillverdenen på eksterne servere, noe som betyr at fremgangen stopper i det øyeblikket spilleren mister internettforbindelsen. Denne oppgaven undersøker hvordan den begrensningen kan overkommes — nærmere bestemt hvordan et spill kan fortsette å kjøre uten nett og deretter trygt slå sammen det som skjedde med den delte verdenen når spilleren kobler seg til igjen.

Kjerneide en er å registrere hver spillerhandling som et lite, selvstendig kommandoobjekt i stedet for å skrive direkte til serveren. Mens spilleren er frakoblet, samles disse kommandoene opp lokalt. Når forbindelsen er gjenopprettet, spilles de av mot den autoritative serveren i riktig rekkefølge. Det vanskelige er å avgjøre hva som skal skje når serverens verden har endret seg i mellomtiden: en annen spiller kan ha revet ned den samme bygningen eller inntatt det samme territoriet. Oppgaven løser dette gjennom en konflikthåndteringsstrategi som klassifiserer kommandoer etter type — noen kan alltid anvendes uavhengig av hva som har endret seg, andre krever en versjonssjekk, og noen må rett og slett avvises og rapporteres tilbake til spilleren.

En prototype ble bygget i C++ for å validere designet, inkludert en SQLite-basert frakoblet database og en holdbar kommandologg som overlever krasj. Alle komponenter er testet med 31 automatiserte tester.
\pagenumbering{roman}
\newpage
