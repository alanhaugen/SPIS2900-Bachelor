...
A server-authoritative persistence architecture requires continuous internet connectivity to a centralized cloud-based server. Although this model guarantees world consistency for all players in online multiplayer games, it entails significant drawbacks when applied to offline or hybrid scenarios:

Since all state-modifying actions are executed as remote transactions, gameplay cannot progress without server access. This requirement entirely prevents offline play and restricts deployment flexibility on platforms where online access is limited or requires a paid subscription. If a player loses internet connectivity mid-session, whether due to router failure, mobile network dropout, or server maintenance, gameplay cannot continue. Any progress made while offline is lost, the client has no way to queue actions for later submission. For players from regions with unreliable internet connectivity, the game becomes more or less inaccessible. The combined architecture described in Section~\ref{sec:architecture} tackles these limitations by decoupling action generation from action transmission.

In a purely server-authoritative model, the client does not maintain an authoritative local model of persistent world state. While a temporary state may exist in memory during a session, durable storage is managed exclusively by the remote infrastructure. This design prevents local world progression during disconnection and complicates support for local cooperative play modes. Implementing a local persistence layer, such as the SQLite-backed DBI described in Section~\ref{sec:sqliteimp}, enables progress to persist across process termination by writing state to disk before the server confirms.

A server-authoritative architecture assumes a single linear progression of state changes and does not accommodate concurrent or divergent modifications in separate environments. For example, both an offline client and the live server may modify the same entity independently. As a result, such architectures usually lack:

\begin{enumerate}
  \item \textbf{Version tracking mechanisms}: Without a record of which version of an entity a player acted on, it is impossible to determine whether a subsequent action was based on outdated information.
  \item \textbf{Conflict identification strategies}: Without version tracking, there is no reliable method to identify when two independent modifications have targeted the same entity in incompatible ways.
  \item \textbf{Deterministic merge policies}: Even when a conflict is detected, a defined rule for resolution is required. Approaches such as last-write-wins, rejection, or type-specific merging each produce different outcomes for the player.
\end{enumerate}

Without these components, safe synchronization of locally modified state with cloud-based state is not possible. The version-based conflict recognition and command classification strategy described in Section~\ref{sec:conflictresolution} directly addresses all three gaps.
