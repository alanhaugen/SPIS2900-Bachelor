...
\subsection*{How can a offline-online synchronization system for game persistence targeting survival multiplayer games be designed and evaluated with respect to consistency, conflict resolution, and reconnection performance?}
...
\begin{enumerate}
  \item \textbf{Version tracking mechanisms} — without a record of which version of an entity a player acted on, it is impossible to determine whether a subsequent action was based on stale information.
  \item \textbf{Conflict detection strategies} — without version tracking, there is no reliable way to identify when two independent modifications targeted the same entity in incompatible ways.
  \item \textbf{Deterministic merge policies} — even when a conflict is detected, there must be a defined rule for resolving it; last-write-wins, rejection, or type-specific merging each produce different outcomes for the player.
\end{enumerate}

Without these components, safely synchronizing locally modified state with cloud-based state is not possible. The version-based conflict detection and command classification strategy developed in Section~\ref{sec:conflictresolution} directly addresses each of these three gaps.

\subsubsection{Scalability vs.\ Flexibility Trade-off}

A centralized server-authoritative design scales well for persistent multiplayer environments but sacrifices flexibility. Extending it toward hybrid offline–online play requires introducing distributed state management — concerns that a purely server-authoritative architecture was not originally designed to address. The tension is not simply technical: each step toward offline support requires loosening assumptions that the rest of the system depends on. Allowing local writes means accepting that two versions of the same entity can exist simultaneously. Accepting divergent state means requiring conflict resolution. Requiring conflict resolution means classifying every operation by its merge semantics. These are compounding concerns, and retrofitting them onto an architecture built around a single authoritative source of truth is substantially more difficult than designing for them from the outset.
...
To enable offline progression while preserving compatibility with a server-authoritative model, this thesis proposes a command-based persistence mechanism inspired by the Command Pattern as described in Design Patterns: Elements of Reusable Object-Oriented Software \autocite{gamma1994design} and further discussed in Game Programming Patterns \autocite{nystrom2014game}.
...
The network latency model shows that even on a poor mobile connection (300 ms RTT, 5\% packet loss), a 10,000-command log reconnects in approximately 356 ms end-to-end. For connections below roughly 40 ms RTT, local processing cost dominates; above that threshold network latency takes over. The practical implication is that coalescing commutative deltas to reduce log size is a more effective optimisation than improving replay speed for players on high-latency connections.
...
This thesis investigated the following research question: How can a offline-online synchronization system for game persistence targeting survival multiplayer games be designed and evaluated with respect to consistency, conflict resolution, and reconnection performance?
