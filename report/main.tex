...
Many survival games store their world entirely on remote servers. This means they can not play the game offline, and if the developer goes bankrupt or stops the online service, the game is lost forever. This thesis looks at ways to make a system which works both for offline and offline gaming sessions, and which allows users to connect online from an offline session when they feel like it.

The core idea is to record each player action as a command rather than immediately sending changes to the server. While offline, these commands queue up locally. When connectivity returns, they are replayed against the authoritative server in order. Conflict resolution becomes a challenge, how should the game respond when another player may have demolished the same building, or claimed the same territory. The thesis addresses this through a conflict resolution strategy that classifies commands by type, some can always be applied regardless of what changed, some need a version check, and some must simply be declined and reported back to the player.

A prototype was built in C++ to validate the design, including a SQLite-backed offline database and a durable command log that survives crashes. 31 automated tests have been written to test the system.

\section*{Sammendrag}

Mange overlevelsesspill lagrer spillverdenen på eksterne servere. Dette gjør det umulig å spille spillet offline, og hvis utvikleren går konkurs så mister spillerene tilgang til spillet.  Denne oppgaven undersøker hvordan et system kan utvikles som gjør at en kan spillet spillet uten internett, og så synkronisere spillverdenen ved å koble seg til en online server når en spiller selv ønsker det.

Forslaget er å registrere hver spillerhandling som et kommando, i stedet for å sende ny tilstand direkte til serveren. Mens spilleren er frakoblet, samles disse kommandoene opp lokalt. Når forbindelsen er gjenopprettet, spilles de av mot den autoritative serveren i riktig rekkefølge. Det vanskelige er å avgjøre hva som skal skje når serverens verden har endret, konflikthåndtering blir viktig. Hvordan skal en spiller som har revet ned den samme bygningen eller inntatt det samme territoriet håndteres. Oppgaven løser dette med en konflikthåndteringsstrategi som klassifiserer kommandoer etter type, noen kan alltid anvendes uavhengig av hva som har endret seg, andre krever en versjonssjekk, og noen må rett og slett avvises og rapporteres tilbake til spilleren.

En prototype ble bygget i C++, inkludert en SQLite-basert frakoblet database og en kommandoliste som overlever at spillet plutselig lukkes. 31 automatiserte tester ha blitt skrevet for å teste systemet.
\pagenumbering{roman}
\newpage
...
To reproduce the results, the code artifacts for the project are provided: \url{https://github.com/alanhaugen/SPIS2900-Bachelor}
...
\begin{itemize}
  ...
  \item \texttt{Command} and \texttt{CommandLog} — the in-memory command queue described in Section~3.1.
  ...
  \item \texttt{ReplayCommand} / \texttt{ReplayAll} — the conflict-resolution algorithm from Section~3.3, including the \texttt{RejectionManifest} type.
\end{itemize}
